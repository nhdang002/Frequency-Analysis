...
#Văn bản được mã hóa 
encrypted_message = "ytn xqavhq yzhu  xu qzupvd ltmat qnncq vgxzy hmrty vbynh ytmq ixur qyhvurn\
...
#Tôi sẽ sử dụng một từ điển để lưu trữ các biến trong  từ điển này
# các chữ cái được lưu trữ, sử dụng nó để đếm tần 
#suất mà các ký tự khác nhau được sử dụng trong message
stored_letters = {} 
...
for char in encrypted_message:
    if char not in stored_letters: #nếu chuỗi đó k có trong stored_letters thì chúng ta đặt nó vào các chữ cái được lưu trữ. Nếu message đó chưa có trong các stored_letters
         stored_letters[char] = 1  #nó sẽ đưa nó vào bên trong được lưu trữ với giá trị là 1
    else: #ngược lại nó sẽ tăng kí tự trong đó lên 1 và sau đó in ra các chữ cái trong đó được lưu trữ
         stored_letters[char] +=1

print (stored_letters) #in ra tần suất xuất hiện
# {'y': 373, 't': 183, 'n': 488, ' ': 741, 'x': 291, 'q': 276, 'a': 116, 'v': 348, 'h': 235, 'z': 95, 'u': 280, 'p': 156, 
# 'd': 59, 'l': 90, 'm': 264, 'c': 104, 'g': 83, 'r': 82, 'b': 83, 'i': 166, 'e': 76, 's': 19, 'f': 49, 'o': 4, 'k': 5, 'j': 5, 'w': 1}


#Phân tích tần suất chữ cái và thay thế giải mã bằng chữ cái
#sử dụng Opening code đồng màu ASCII
attempt = encrypted_message.replace("v", "A")
attempt = attempt.replace("n", "E")
attempt = attempt.replace("y", "T")
attempt = attempt.replace("v", "A")
attempt = attempt.replace("x", "O")
attempt = attempt.replace("q", "S")
attempt = attempt.replace("m", "I")  
attempt = attempt.replace("u", "N")
attempt = attempt.replace("d", "Y")
attempt = attempt.replace("h", "R")
attempt = attempt.replace("t", "H")
attempt = attempt.replace("p", "D")
attempt = attempt.replace("a", "C")
attempt = attempt.replace("g", "B")
attempt = attempt.replace("z", "U")
attempt = attempt.replace("c", "M")
attempt = attempt.replace("l", "W")
attempt = attempt.replace("r", "G")
attempt = attempt.replace("b", "F")
attempt = attempt.replace("i", "L")
attempt = attempt.replace("e", "P")
attempt = attempt.replace("s", "K")
attempt = attempt.replace("f", "V")
attempt = attempt.replace("o", "J")
attempt = attempt.replace("j", "Q")
